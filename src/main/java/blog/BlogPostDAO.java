...
public class BlogPostDAO {
	...
	// changes to templates and post retrieval code.
        ...
        DBObject dbObject = findByPermalink(permalink);
        if (dbObject != null) {
            List<DBObject> comments = (List<DBObject>) dbObject.get("comments");
            DBObject comment = comments.get(ordinal);

            if (!comment.containsField("num_likes")) {
                comment.put("num_likes", 0);
            } else {
                Integer likes = (Integer) comment.get("num_likes");
                comment.put("num_likes", ++likes);
            }

            WriteResult result = postsCollection.update(new BasicDBObject("permalink", permalink),
                    dbObject, false, false);
        }
    }
}
